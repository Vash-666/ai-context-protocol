...
def analyze_traces(traces: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for agent, data in summary["agents"].items():
        ...
        if latencies:
            data["p50_latency_ms"] = round(statistics.median(latencies), 1)
            data["p95_latency_ms"] = round(
                statistics.quantiles(latencies, n=20)[18] if len(latencies) >= 20 else max(latencies), 1
            )
            data["avg_latency_ms"] = round(statistics.mean(latencies), 1)
